...
for stmt in "${TEST_IMPORTS[@]}"; do
    ERR=$(.venv/bin/python -c "$stmt" 2>&1 1>/dev/null) || true
    if GOOGLE_TTS_SERVICE_ACCOUNT_JSON=test .venv/bin/python -c "$stmt" 2>/dev/null; then
        pass "import: ${stmt%%;*}"
    else
...
echo "── 3. Pytest suite ──"
cd "$PB_LIBEXEC"
PYTEST_OUT=$(GOOGLE_TTS_SERVICE_ACCOUNT_JSON=test .venv/bin/python -m pytest tests/ -q 2>&1 || true)
