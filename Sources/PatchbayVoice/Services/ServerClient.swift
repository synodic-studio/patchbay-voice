...
struct ServerClient {
    ...
    func sendTurn(chatID: String, audioData: Data) async throws -> TurnResponse {
        let boundary = UUID().uuidString
        var req = URLRequest(url: baseURL.appending(path: "/api/talk"))
        req.httpMethod = "POST"
        req.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        req.httpBody = multipartBody(boundary: boundary, chatID: chatID, audioData: audioData)
        let (data, _) = try await URLSession.shared.data(for: req)
    ...
    private func multipartBody(boundary: String, chatID: String, audioData: Data) -> Data {
        var body = Data()
        let crlf = "\r\n"
